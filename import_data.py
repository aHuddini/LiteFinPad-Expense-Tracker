"""Import expense data from JSON backup files with validation and merge support."""
...
class DataImporter:
    """Import expense data from JSON backup files."""
    
    def __init__(self, expense_tracker, status_callback=None):
        """Initialize importer with expense tracker and optional status callback."""
    ...
    def import_from_json_backup(self, filepath: Optional[str] = None) -> bool:
        """Import data from JSON backup file. Shows file picker if filepath is None."""
        try:
            if not filepath:
    ...
    def restore_all_months(self, backup_data: dict, merge_mode: str = "merge") -> bool:
        """Restore months from backup. merge_mode='merge' combines with existing data."""
        try:
            restored_months = []
    ...
    def merge_expenses(self, existing: List[Dict], backup: List[Dict]) -> List[Dict]:
        """Merge two expense lists, avoiding duplicates (same date, amount, description)."""
